...
$(function() {
  ...
  $('form.cart .cart__qty-input').on('change',function() {
...
var defaults={url:false,callback:false,target:false,duration:120,on:"mouseover",touch:true,onZoomIn:false,onZoomOut:false,magnify:1};
        ...
        $.ajax({
          ...
          success: function (data) {
            $cf.fadeOut("slow", function(){
              $cf.prev('.message').html("translation missing: en.general.newsletter_form.success_text");
            });
          },
          error: function(data) {
  ...
  function refreshCart(cart) {
    ...
    if (cart.item_count == 0) {
      ...
      $.each(cart.items, function(index, item) {
        ...
        cart_items_html += '<span aria-label="Decrease quantity button" class="ss-icon product-minus js-change-quantity" data-func="minus"><span class="icon-minus"></span></span>';
        cart_items_html += '<input type="number" aria-label="Quantity input text field" min="0" class="quantity" name="updates[]" id="updates_' + item.id + '" value="' + item.quantity +'" data-line-id="' + line_id +'"';
        if(item.requires_shipping == false) {
          cart_items_html += ' disabled="disabled" ';
        } 
        cart_items_html += ' />';
        if(item.requires_shipping == true) {
          cart_items_html += '<span aria-label="Increase quantity button" class="ss-icon product-plus js-change-quantity" data-func="plus"><span class="icon-plus"></span></span>';
        }
        cart_items_html += '</div>';
        cart_items_html += '</div>';
      ...
      cart_action_html += '<span>Cart Subtotal: </span>' + '<span class="money">' + Shopify.formatMoney(cart.total_price, $('body').data('money-format')) + '</span>';
  ...
  // Update cart qty using + and - controls
  $('body').on('click', '.js-change-quantity', function() {
    var dir = $(this).data('func');
    var input = $(this).closest('.product-quantity-box').find('input');
    var curVal = parseInt(input.val());
    var newVal = (dir === 'plus') ? 1 : -1;

    $(this).removeClass('js-change-quantity');
    input.val(curVal + newVal).trigger('change');
  });
...
$(document).ready(function() {
  ...
  $('button, input, textarea, select').blur(function(){
...
/*
Mobile App URL replacement and deep linking
=============================================*/
function get(name){
...
$(document).ready(function() {
  var appLibraryLink = '/pages/contact-us/';
  console.log('cookies? '+typeof Cookies);
  if(get('lfa') == '1') {
    //link detected, add cookie   
    console.log('found inbound app link');
    Cookies.set('userHasNativeApp', '1')
  }
  if(Cookies.get('userHasNativeApp') == '1') {
    console.log('userHasNativeApp cookie found');
    // ADD THIS BACK TO ENABLE $('a[href^="https://digitallibrary.soundstrue.com"]').attr('href',appLibraryLink);
  }
});
